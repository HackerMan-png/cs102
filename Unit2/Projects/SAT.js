...
for(i in col){
  for(l in col[i]){
    col[i][l] = Number(col[i][l])
    temp += col[i][l]
  }
  totalAverage += temp/10
  schoolAvgs.push(temp/10)
  temp = 0
}
...
// for(s in col){
//   let max = Math.max(...col[s])
//   let min = Math.min(...col[s])
...
let max = Math.max(...schoolAvgs)
let min = Math.min(...schoolAvgs)
let percentArr = []

for(i in schoolAvgs){
  let difference = max - min
  let tempDif = schoolAvgs[i] - min
  tempDif /= difference
  tempDif *= 0.5
  tempDif += 0.5
  tempDif *= 100
  tempDif = tempDif.toFixed(2)
  percentArr.push(tempDif)
}

// console.table(percentArr)
let percentArr2 = new Array(50);
for(let i = 0; i < 50; i++){
  percentArr2[i] = new Array(10);
}
...
for(i in col){
  for(j in col[i]){
    ...
    for(a in col){
      ...
      // percentArr2[i][j] = percentArr2[i][j]
    }
    percentArr2[i][j] = Math.ceil(greater/5)
  }
}

    console.table(percentArr2);
