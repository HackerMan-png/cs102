...
function getQuad(board, quadNum) {
  //creates an array of all of the array elements in the same quadrant
  let inQuad = [];
...
//Filters through all of the used values and return only the possible values for that cell
function getPossible(board, row, col) {
  ...
  //every row element and say what isn't possible
  for (r in inRow) {
    let included = false
  ...
  //every col element and say what isn't possible
  for (c in inCol) {
    let included = false
    for (p in notPossible) {
...
while (running) {
  ...
  for (row2 in _BOARD) {
    for (col2 in _BOARD[row2]) {
      ...
      if (getPossible(_BOARD, row2, col2).length == 0) {
        filled++
        if (filled == 81) {
          running = false
        }
